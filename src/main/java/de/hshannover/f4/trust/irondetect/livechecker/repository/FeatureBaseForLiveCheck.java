 ...
 *
 */
package de.hshannover.f4.trust.irondetect.livechecker.repository;
...
public class FeatureBaseForLiveCheck {
	...
	public static synchronized FeatureBaseForLiveCheck getInstance() {
		if (instance == null) {
			instance = new FeatureBaseForLiveCheck();
		}
	...
	public Event addNewFeatures(List<Pair<String, Pair<Feature, Boolean>>> features, boolean isTraining) {
		if (features != null && features.size() > 0) {
			...
			for (Pair<String, Pair<Feature, Boolean>> devicePair : features) {
				...
				currentFeature = currentPair.getFirstElement();
				wasDeleted = currentPair.getSecondElement();

				// try to use context parameter for feature date+time, use
				// current time otherwise
				ContextParameter ctxParamDateTime = currentFeature
						.getContextParameterByType(new ContextParamType(
								ContextParamType.DATETIME));
				currentFeatureDateTime = ctxParamDateTime != null ? Helper
						.getXsdStringAsCalendar(ctxParamDateTime.getValue())
						...
						if ((currentChangesForDevice = changesDuringFeatureBaseUpdate
								.get(device)) == null) {
							currentChangesForDevice = new HashSet<String>();
							changesDuringFeatureBaseUpdate.put(device,
									currentChangesForDevice);
						}
						...
						if ((currentNumberOfDeletedFeatures = numberOfDeletedFeatures
								.get(device)) == null) {
							currentNumberOfDeletedFeatures = 0;
							numberOfDeletedFeatures.put(device,
									currentNumberOfDeletedFeatures);
						}

						if ((currentNumberOfNewFeatures = numberOfNewFeatures
								.get(device)) == null) {
							currentNumberOfNewFeatures = 0;
							numberOfNewFeatures.put(device, currentNumberOfNewFeatures);
						}

						if ((currentNumberOfUpdatedFeatures = numberOfUpdatedFeatures
								.get(device)) == null) {
							currentNumberOfUpdatedFeatures = 0;
							numberOfUpdatedFeatures.put(device,
									currentNumberOfUpdatedFeatures);
						}

						/**
						 * Check if time of current Feature is before/after the saved
						 * value for the specific training data for the Feature's
						 * device.
						 */
						...
						if (wasDeleted == true) {
							fh = getFeatureHistoryByQualifiedFeatureId(
									featureHistoryForCurrentDevice,
									currentFeature.getQualifiedId());
							if (fh != null) {
								fh.addFeature(currentPair);
								currentChangesForDevice.add(currentFeature
										.getQualifiedIdWithoutInstance());
								currentNumberOfDeletedFeatures++;
							} else {
			...
			TrainingData t;
			for (String d : changesDuringFeatureBaseUpdate.keySet()) {
				logger.info("Features for device '" + d
						+ "' (deleted/new/updated): "
						+ numberOfDeletedFeatures.get(d) + "/"
						+ numberOfNewFeatures.get(d) + "/"
						+ numberOfUpdatedFeatures.get(d));

				if (isTraining) {
					changedFeaturesForDevice = changesDuringFeatureBaseUpdate
							.get(d);
					t = trainingData.get(d);
					t.setFeatureIds(changedFeaturesForDevice);
					logger.info("Time of first feature for device '" + d
							+ "': "
							+ Helper.getCalendarAsXsdDateTime(t.getStartTime()));
					logger.info("Time of last feature for device '" + d + "': "
							+ Helper.getCalendarAsXsdDateTime(t.getEndTime()));
				}
			}
			...
			if (isTraining) {
				...
				/**
				 * Sends a Map containing Lists of FeatureIds that changed
				 * during the FeatureBaseUpdate for each device.
				 */
				e = new FeatureBaseUpdateEvent();
				((FeatureBaseUpdateEvent) e)
	...
	public Pair<Feature, Boolean> getCurrentSpecificFeatureForDevice(
		...
		return getFeatureHistoryByQualifiedFeatureId(
				this.featuresForDevice.get(device), featureId)
				.getLatestFeature();
	}
	...
	public synchronized List<Feature> getFeaturesByContext(String device,
		...
		List<FeatureHistory> featureHistoriesForDevice = this.featuresForDevice
				.get(device);
		...
		for (String featureId : ids) {
			...
			for (FeatureHistory fh : featureHistoriesForDevice) {
				if (fh.getQualifiedFeatureHistoryIdWithoutInstance().contains(
						lowercaseFeatureId)) {
	...
	private List<Feature> getFeaturesForContext(FeatureHistory history) {
		...
		logger.trace("history: " + history.getAllFeatures().size() + " for featureID: "
				+ history.getQualifiedFeatureHistoryId());
